$(document).ready(function(){
	...
	$('.apply-label').live('click', function() {
	...
	$('#manage-existing-labels').live('click', function() {
		listLabels(true);
	});
	...
	// Show a live preview.
	$("#new-label-name").live('keyup', function() {
		if($("#new-label-name").val().length == 0) {
			$("#label-preview").text('Label Preview');
		} else {
...
function saveNewLabel() {
	...
	if($("#new-label-name").val().length > 40) {
		...
		$.ajax({
			type: 'POST',
			url: '/admin/plugin/li3b_blog/labels/create.json',
...
function listLabels(manage) {
	$("#current-labels").fadeOut('medium', function() {
		$.get('/admin/plugin/li3b_blog/labels/index.json', function(data) {
			if(data.success == true) {
				var labelHtml = '';
				if(manage == true) {
					// Manage labels
...
function deleteLabel(label) {
	$.ajax({
		type: 'POST',
		url: '/admin/plugin/li3b_blog/labels/delete/' + label + '.json',
		data: {},
		success: function(data) {
			if(data.success == true) {
